...
class Plugin_Starter_Admin
{
	...
	public function enqueue_scripts()
	{
		...
		if ($current_screen->id == 'plugin-starter_page_plugin-starter-react') {
			wp_enqueue_script(
	...
	public function plugin_starter_admin_menu()
	{
		add_menu_page(
			...
			'manage_options',
			$this->plugin_name,
			array($this, 'plugin_starter_dashboard_php_page_html'),
			plugin_dir_url(__DIR__) . 'admin/images/menu-icon.svg',
			57
		);
		add_submenu_page(
			$this->plugin_name,
			esc_html__('PHP Page', 'plugin-starter'),
			esc_html__('PHP Page', 'plugin-starter'),
			'manage_options',
			$this->plugin_name,
			array($this, 'plugin_starter_dashboard_php_page_html')
		...
		add_submenu_page(
			$this->plugin_name,
			esc_html__('React Page', 'plugin-starter'),
			esc_html__('React Page', 'plugin-starter'),
			'manage_options',
			$this->plugin_name . '-react',
	...
	*/
	...
	public function rest_plugin_starter_update_options(WP_REST_Request $request) //WP_REST_Request $request
	{
		...
		$plugin_starter_options = map_deep(wp_unslash($request->get_param('plugin_starter_options')), 'sanitize_text_field');
